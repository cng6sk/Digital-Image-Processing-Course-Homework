...
def DCP(src):
    ...
    return (J * 255).astype(np.float64), (dark * 130).astype(np.float64)
...
if __name__ == '__main__':
    ...
    # src = cv2.imread("img/input7.png").astype(np.float64)
    src = cv2.imread("img/input8.jpg").astype(np.float64)
    # src = cv2.imread("img/02_outdoor_GT.jpg").astype(np.float64)
    src2 = cv2.imread("img/input3.jpg").astype(np.float64)
    J, dark = DCP(src)
    ...
    # 合并输入图像和暗通道图像
    dark_bgr = cv2.merge([dark, dark, dark])
    combined = np.hstack((src.astype(np.uint8), dark_bgr.astype(np.uint8)))
